...
public:
	...
	// Initialization
	void initialize(MovementComponent* moveComp, float mass = 1.f, XMFLOAT3 acceleration = XMFLOAT3(0.f, 0.f, 0.f), XMFLOAT3 deceleration = XMFLOAT3(0.f, 0.f, 0.f))
	{
		m_moveComp = moveComp;
		m_mass = mass;
	...
	void addForceDir(Direction dir, float dt, float multiplier = 1.f)
	{
		...
		case Direction::FORWARD:
			finalForce = XMVectorScale(m_moveComp->forward, m_acceleration.x * multiplier);
			break;
		case Direction::BACKWARD:
			finalForce = XMVectorScale(m_moveComp->backward, m_acceleration.x * multiplier);
			break;
		case Direction::LEFT:
			finalForce = XMVectorScale(m_moveComp->left, m_acceleration.x * multiplier);
			break;
		case Direction::RIGHT:
			finalForce = XMVectorScale(m_moveComp->right, m_acceleration.x * multiplier);
			break;
		case Direction::UP:
			finalForce = XMVectorScale(m_moveComp->up, m_acceleration.y * multiplier);
			break;
		case Direction::DOWN:
			finalForce = XMVectorScale(m_moveComp->down, m_acceleration.y * multiplier);
			break;
		default:
	...
	void addForceDir(XMVECTOR dir, float dt, float multiplier = 1.f)
	{
		...
		finalForce = XMVectorScale(dir, m_acceleration.x * multiplier);
	...
	void updatePosition(float dt, bool isCamera = false)
	{
		...
		m_velocity.y *= m_deceleration.y;
		m_velocity.z *= m_deceleration.z;
	}
